...
function doGet(e) {
  ...
  var ruta = params.v || 'buscar'; // Por defecto busca si no se especifica ruta
  var termino = params.q || '';
  
  // Si la petición pide buscar (o entra por defecto)
  if (ruta === 'buscar' || ruta === 'api' || ruta === '') {
...
function buscarPublicacion(termino) {
  ...
  var cacheKey = "busqueda_v14_" + encodeURIComponent(termino.toLowerCase());
  ...
  // Usamos la primera pestaña directamente para evitar fallos de nombre
  ...
  for (var c = 0; c < cabecera.length; c++) {
    ...
    if (nombreCol === 'sa' || nombreCol.indexOf('url') !== -1) colUrlSA = c;
    if (nombreCol === 'timestamp' || nombreCol === 'ry' || nombreCol.indexOf('date') !== -1) colFechaRY = c;
    if (nombreCol === 'caption' || nombreCol === 'texto') colTexto = c;
  }
  ...
  for (var i = 1; i < datos.length; i++) {
    ...
    // Búsqueda de texto segura (si la columna B está vacía, busca en la fila)
    var textoCompleto = "";
    if (fila[colTexto] && fila[colTexto].toString().trim() !== "") {
      textoCompleto = fila[colTexto].toString();
    } else {
      for (var col = 0; col < fila.length; col++) {
        if (fila[col] && fila[col].toString().length > 15) {
          textoCompleto = fila[col].toString();
          break;
        }
      }
    }

    if (!textoCompleto || textoCompleto.trim() === "") continue;
    ...
    if (tiempoMilisegundos === 0) tiempoMilisegundos = datos.length - i; 
    
    var urlPost = (colUrlSA !== -1 && fila[colUrlSA]) ? fila[colUrlSA].toString().trim() : "#";
    if (urlPost === "" || !urlPost.startsWith("http")) urlPost = "#";

    var fotoMain = (colFotoPrincipal < fila.length && fila[colFotoPrincipal]) ? fila[colFotoPrincipal].toString().trim() : "";
  ...
  for (var idx = 0; idx < limiteResultados; idx++) {
    ...
    for (var k = 0; k < indicesFotosCarrusel.length; k++) {
      var urlChild = (indicesFotosCarrusel[k] < item.filaRaw.length && item.filaRaw[indicesFotosCarrusel[k]]) ? item.filaRaw[indicesFotosCarrusel[k]].toString().trim() : "";
